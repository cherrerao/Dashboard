...
function renderDetalleDashboard(data) {
  ...
  detalleSelected = detalleEstab[0] || '';
...
function onEstabChange(val) {
  detalleSelected = val;
  // Mostrar/ocultar col establecimiento
  const col = document.getElementById('dt-estab-col');
...
function mkTopSobBar(id, items, mesesField, label) {
  ...
  detalleCharts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: { color: '#8889a6', font: { size: 10 } },
  ...
  detalleCharts['dc-redes'] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ticks: { color: '#8889a6', font: { size: 11 }, callback: v => v + '%' },
