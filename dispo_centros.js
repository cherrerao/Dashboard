...
function renderCentrosDashboard(data) {
  ...
  centrosSelected = centrosEstab[0] || '';
...
function onCentrosEstabChange(val) {
  centrosSelected = val;
  const col = document.getElementById('ct-estab-col');
  if (col) col.style.display = val ? 'none' : '';
  ...
  if (ctx1) {
    centrosCharts['cc-ind'] = new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [{
          data: dVals,
          backgroundColor: dLabels.map(l => CND_COLORS[l]?.bg  || 'rgba(136,137,166,0.7)'),
          borderColor:     dLabels.map(l => CND_COLORS[l]?.border || '#8889a6'),
      ...
      options: {
        ...
        plugins: {
          legend: { position:'right', labels:{ color:'#8889a6', font:{ size:10 }, padding:10 } },
  ...
  if (ctx2) {
    centrosCharts['cc-tipo'] = new Chart(ctx2, {
      ...
      options: {
        ...
        plugins: { legend:{ labels:{ color:'#8889a6', font:{ size:11 } } } },
        scales: {
          x:{ stacked:true, ticks:{ color:'#8889a6', font:{ size:10 } }, grid:{ color:'rgba(255,255,255,0.04)' } },
          y:{ stacked:true, ticks:{ color:'#8889a6', font:{ size:11 } }, grid:{ color:'rgba(255,255,255,0.06)' }, beginAtZero:true }
        },
        onClick: (e, elems) => {
          if (!elems.length) return;
  ...
  if (ctx4) {
    centrosCharts['cc-cobertura'] = new Chart(ctx4, {
      ...
      options: {
        ...
        scales: {
          x:{ ticks:{ color:'#8889a6', font:{ size:10 } }, grid:{ color:'rgba(255,255,255,0.04)' } },
          y:{ ticks:{ color:'#8889a6', font:{ size:11 } }, grid:{ color:'rgba(255,255,255,0.06)' }, beginAtZero:true }
        },
        onClick: (e, elems) => {
          if (!elems.length) return;
    ...
    if (topSob.length) {
      ...
      centrosCharts['cc-top-sob'] = new Chart(ctx5, {
        ...
        options: {
          ...
          scales: {
            x:{ ticks:{ color:'#8889a6', font:{ size:10 } }, grid:{ color:'rgba(255,255,255,0.06)' },
               title:{ display:true, text:'Meses de cobertura', color:'#8889a6', font:{ size:10 } } },
            y:{ ticks:{ color:'#ecedf5', font:{ size:10 } }, grid:{ color:'rgba(255,255,255,0.03)' } }
          },
          onClick: (e, elems) => {
            if (!elems.length) return;
...
function renderCentrosRedesChart() {
  ...
  centrosCharts['cc-redes'] = new Chart(ctx, {
    ...
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins: {
        legend:{ labels:{ color:'#8889a6', font:{ size:11 } } },
        tooltip:{ callbacks:{ label: c => ` ${c.raw}%` } }
      },
      scales: {
        x:{ ticks:{ color:'#ecedf5', font:{ size:11 } }, grid:{ color:'rgba(255,255,255,0.04)' } },
        y:{ ticks:{ color:'#8889a6', font:{ size:11 }, callback: v=>v+'%' },
           grid:{ color:'rgba(255,255,255,0.06)' }, beginAtZero:true, max:100 }
      },
      onClick: (e, elems) => {
        if (!elems.length) return;
...
function renderCentrosEstabsChart() {
  ...
  centrosCharts['cc-estabs'] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x:{ ticks:{ color:'#8889a6', font:{ size:10 }, callback: v=>v+'%' },
           grid:{ color:'rgba(255,255,255,0.06)' }, max:100 },
        y:{ ticks:{ color:'#ecedf5', font:{ size:10 } }, grid:{ color:'rgba(255,255,255,0.03)' } }
      },
      onClick: (e, elems) => {
        if (!elems.length) return;
...
function renderCentrosTable() {
  ...
  const showEstab = !centrosSelected;
  ...
  document.getElementById('ct-body').innerHTML = !slice.length
    ...
    <tr>
      ...
      ${showEstab ? `<td style="font-size:11px;color:var(--muted2);max-width:160px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${r.establecimiento}</td>` : '<td style="display:none"></td>'}
